...
namespace ReportMakerSD
{
    ...
    {
        public static string RunReportThree(DateTime dateTimeTo)
        {
            ...
            var SDR0Data0Sorted = MainForm.SDR0Data0.OrderBy(x => x.OAreaApplicant).ThenBy(x => x.Org).ToList();
            ...
            using (var package = new ExcelPackage())
            {
                ...
                {
                    if ((row.Rating == "Удовлетворительное" || row.Rating == "Низкое") && row.OAreaApplicant == "ОЗ ОДУ Северо-Запада")
                    {
                        ...
                        worksheet1.Cells[2 + count1, 11].Value = row.DateAnswer;
                        worksheet1.Cells[2 + count1, 12].Value = row.AnswerText;
                        worksheet1.Cells[2 + count1, 13].Value = row.Rating;
                        count1++;
                    }
                }

                // Красим 1 лист
                ExcelRange rg1 = worksheet1.Cells[1, 1, count1 + 1, 13];
                ...
                #region Второй лист
                ExcelWorksheet worksheet2 = package.Workbook.Worksheets.Add("Количество обращений по ОЗ");
                int count2 = 1, count2_2 = 0;
                ...
                worksheet2.Cells[2, 1, count2, 2].Sort(1);

                // Красим 2 лист
                ExcelRange rg2 = worksheet2.Cells[1, 1, count2 + 2, 2];
                ExcelTable tab2 = worksheet2.Tables.Add(rg2, "Table2");
                tab2.TableStyle = TableStyles.Medium13;

                worksheet2.Cells[1, 1].Value = "Оперзона";
                worksheet2.Cells[1, 2].Value = "Количество";
                count2++;
                worksheet2.Cells[count2 + 1, 1].Value = "Общий итог";
                worksheet2.Cells[count2 + 1, 2].Formula = "=SUM(" + worksheet2.Cells[2, 2].Address + ":" + worksheet2.Cells[count2, 2].Address + ")";
                ...
                worksheet2.Column(2).Width = 16;
                worksheet2.Row(count2 + 1).Style.Font.Bold = true;
                ...
                worksheet2.HeaderFooter.OddHeader.CenteredText = "&24&U&\"Arial,Regular Bold\"Количество обращений по ОЗ";
                #endregion

                #region Заполняем 3 лист
                ExcelWorksheet worksheet3 = package.Workbook.Worksheets.Add("Количество по группам услуг");
                //var SDR0Data0Sorted3 = SDR0Data0.OrderBy(x => x.OArea).ThenBy(x => x.ServiceGroup).ToList();
                ...
                foreach (var row in SDR0Data0Sorted)
                {
                    ...
                    worksheet3.Cells[count3 + 2, 1].Value = OAreaPrev;
                    for (int i = 0; i < ColumnsReport3.Count; i++)
                    {
                        if (row.ServiceGroup == ColumnsReport3[i])
                        {
                            worksheet3.Cells[count3 + 2, i + 2].Value = worksheet3.Cells[count3 + 2, i + 2].GetValue<int>() + 1;
                        }
                    }
                }
                worksheet3.Cells[count3 + 3, 1].Value = "ИТОГО";
                for (int i = 0; i <= count3 + 1; i++)
                {
                    worksheet3.Cells[i + 1, ColumnsReport3.Count + 2].Formula = "=SUM(" + worksheet3.Cells[i + 1, 2].Address + ":" + worksheet3.Cells[i + 1, ColumnsReport3.Count + 1].Address + ")";
                }

                //// Красим 3 лист
                ExcelRange rg3 = worksheet3.Cells[1, 1, count3 + 3, ColumnsReport3.Count + 2];
                ExcelTable tab3 = worksheet3.Tables.Add(rg3, "Table3");
                tab3.TableStyle = TableStyles.Medium13;

                int countColumns = 2;
                worksheet3.Cells[1, countColumns - 1].Value = "Оперзона";
                foreach (string ColumnsReport in ColumnsReport3)
                {
                    worksheet3.Cells[1, countColumns].Value = ColumnsReport;
                    worksheet3.Cells[1, countColumns].Style.TextRotation = 90;
                    worksheet3.Cells[count3 + 3, countColumns].Formula = "=SUM(" + worksheet3.Cells[2, countColumns].Address + ":" + worksheet3.Cells[count3 + 2, countColumns].Address + ")";
                    countColumns++;
                }
                worksheet3.Cells[count3 + 3, countColumns].Formula = "=SUM(" + worksheet3.Cells[2, countColumns].Address + ":" + worksheet3.Cells[count3 + 2, countColumns].Address + ")";
                worksheet3.Cells[1, countColumns].Value = "ИТОГО";


                worksheet3.Calculate();
                worksheet3.View.FreezePanes(2, 1);
                worksheet3.View.ZoomScale = 100;
                worksheet3.Row(1).Height = 160;
                ...
                worksheet3.Row(count3 + 3).Style.Font.Bold = true;
                worksheet3.Cells.AutoFitColumns();  //Autofit columns for all cells
                worksheet3.Column(1).Width = 30;
                worksheet3.Column(countColumns).Style.Font.Bold = true;
                ...
                worksheet3.HeaderFooter.OddHeader.CenteredText = "&24&U&\"Arial,Regular Bold\"Количество обращений по ОЗ и группам услуг";
                #endregion

                #region Заполняем 4 лист
                ExcelWorksheet worksheet4 = package.Workbook.Worksheets.Add("ФГП по группам услуг");
                var SDR0Data0Sorted4 = MainForm.SDR0Data0.OrderBy(x => x.FSG).ThenBy(x => x.ServiceGroup).ToList();

                int count4 = 0;
                string FSGPrev = "Администраторы Naumen Service Desk";
                foreach (var row in SDR0Data0Sorted4)
                {
                    if (FSGPrev != row.FSG)
                    {
                        count4++;
                    ...
                    worksheet4.Cells[count4 + 2, 1].Value = FSGPrev;
                    for (int i = 0; i < ColumnsReport3.Count; i++)
                    {
                        if (row.ServiceGroup == ColumnsReport3[i])
                        {
                ...
                countColumns = 2;
                worksheet4.Cells[1, countColumns - 1].Value = "ФГП";
                foreach (string ColumnsReport in ColumnsReport3)
                {
                ...
                worksheet4.HeaderFooter.OddHeader.CenteredText = "&24&U&\"Arial,Regular Bold\"Количество обращений по ФГП и группам услуг";
                #endregion

                #region Заполняем 5 лист
                ExcelWorksheet worksheet5 = package.Workbook.Worksheets.Add("Филиалы по группам услуг");
                //var SDR0Data0Sorted4 = SDR0Data0.OrderBy(x => x.FSG).ThenBy(x => x.ServiceGroup).ToList();

                int count5 = 0;
                string OrgPrev = "Исполнительный аппарат";
                foreach (var row in SDR0Data0Sorted)
                {
                    if (OrgPrev != row.Org)
                    {
                ...
                var xlFile = Utils.GetFileInfo("SD. Отчет за период по " + dateTimeTo.ToString("dd.MM.yyyy") + ".xlsx");
