...
// Отчет по подрядным организациям от заявителей ОЗ ОДУ СЗ

namespace ReportMakerSD
{
    public static partial class SDR0
    {
        public static string RunReportTwo(DateTime dateTimeTo, DateTime DateTimeNowWeek)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                foreach (string ColumnsReport in ColumnsReport0)
                {
                    ...
                    if (ColumnsReport.IndexOf("Просро") >= 0)
                    {
                        worksheet3.Cells[1, countColumns + 1].Value = worksheet3.Cells[1, countColumns + 1].Value + " по состоянию на " + Form0.SDR0Data;
                    }
                    countColumns++;
                }
                ...
                {
                    if ((row.OAreaApplicant.IndexOf("ОЗ ОДУ Северо-Запада") >= 0) && (row.FSG.IndexOf("ТО \\") >= 0) && (row.Status == "2 Назначен" || row.Status == "3 Выполняется" || row.Status == "4 В ожидании"))
                    {
                        if ((row.DateCalc <= dateTimeTo) || (row.Expired != "0:00"))
                        {
                            ...
                            worksheet1.Cells[2 + count1, 13].Style.Numberformat.Format = "dd-mm-yy hh:mm";
